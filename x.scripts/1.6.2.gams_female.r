...
lm <- lm[! is.na(lm$f), ]
# initiate plot
pdf('plots/linkage_maps_gams_female.pdf',12,12)
# loop through each lg
for(lg in levels(lm$lg)){
tmp <- lm[lm$lg == lg, ]
# reprune ends cause weird ! 
tmp$m <- prune.ends(tmp$f)
tmp <- tmp[! is.na(tmp$f),]
# loess 
lo1 <- loess(f~pos,data=tmp, span=0.1)
pos.grid <- data.frame('pos'=seq(min(tmp$pos),max(tmp$pos),1000))
lo1.p <- predict(lo1, newdata=pos.grid,se=T)
# monotonic gam spline 
ss.tmp <- smooth.spline(tmp$pos, tmp$f, cv=T)# find k from df 
gam2 <- scam(f ~ s(pos,k = round(ss.tmp$df,0), bs='mpi'), data = tmp) # long step 
saveRDS(gam2, file=paste0('rds/mono_gam_female_lg',lg,'_',round(ss.tmp$df,0),'df.rds'))
gam2.p <- predict(gam2, newdata=pos.grid)
# plot 
plot(f~pos, data=tmp, ylab='f cM', xlab='physical pos',
