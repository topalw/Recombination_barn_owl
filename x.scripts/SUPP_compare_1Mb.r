...
pdf('plots/SUPP_compare1Mb.pdf',12,16)
par(mfrow=c(5,4))
...
for(lg in lgs){
  ss = lms$ss[lms$lg==lg]
  tmp <- d1m[d1m$ss==ss,]
  plot(tmp$CH_cM~tmp$start,type='l',col=palette[1],lwd=4,lty=2,
       ylim=c(0,max(c(tmp$CH_cM,tmp$av,tmp$f,tmp$m),na.rm=T)),
       ylab='cM/Mb',xlab='Physical position',main=paste0('LG',lg))
  lines(tmp$av~tmp$start,col=palette[2],lwd=3,lty=1) 
