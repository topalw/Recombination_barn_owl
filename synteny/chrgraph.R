...
l = args[1] 
  ...
  p <- ggplot() + geom_rect(data = chrinfo, aes(xmin=xoffset, xmax=xoffset+seqlength, ymin=rphyloorder,ymax= rphyloorder+0.15, fill=chrtype), alpha = 0.6) +
    geom_text(data = chrinfo, aes(x=xoffset, y = jitter(rphyloorder + 0.075,amount=0.03), label = gsub('chr','',gsub('LG','',chrinfo$chrname))), hjust = 0, size = 2) + 
    geom_polygon(data = t, aes(x=xcoords,y=ycoords, fill = alntype, group = alnid), alpha = 0.4) +
    scale_y_continuous(breaks = 1:nrow(speciesinfo)+ 0.075, labels = arrange(speciesinfo, rphyloorder+0.075) %>% pull("Common name")) +
...
pdf(paste0(ref,'vs',l,'.pdf'),width=16,height=10)
print(p) 	
#ggplotly(p)
 dev.off()
